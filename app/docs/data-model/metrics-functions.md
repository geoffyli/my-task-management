...
**Source:** `src/lib/metrics.ts` (362 lines)
...
|----------|--------|---------|
| `getTimeRangeStart(range)` | `src/lib/date-utils.ts` | Converts TimeRange ("30d", "90d", "6m", "all") to start Date |
| `isActiveTask(task)` | `src/lib/constants.ts` | Returns true if status is not Done or Cancelled |
